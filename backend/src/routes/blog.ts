...
blogRouter.put('/edit/:id' , authorizationMiddleware , async(c)=>{
    ...
    const updateData : Partial<{title : string,content : string,published:boolean}> = {}

    if(body.title) updateData.title = validatedBody.title
    if(body.content) updateData.content = validatedBody.content
    if(body.published) updateData.published = validatedBody.published
...
blogRouter.get('/home/posts',authorizationMiddleware , async(c) => {
    const prisma = getPrisma(c.env.DATABASE_URL)
    const userId = c.get("userId")
